...
6. В Вашей обособленной сети должен быть настроен DHCP. Вы можете использовать только статические адресса для Ваших стендов, но тема интеграции VMware vSphere с SUSE Rancher для использования только статическоих адресов и автоматического развертывания кластеров RKE выходит за рамки этого вебинара, но если она Вам интересна некоторый пример и ссылку на описание как это сделать Вы можете найти [здесь](https://github.com/dff1980/2020.2-PoC/blob/cc490da8b3d55404004b3e22eda80c3c67a7f3d9/vApp/readme.md#using-a-pool-of-addresses-in-vapp-in-the-sphere).
7. В Вашей обособленной сети должена быть настроена служба синхронизации времени NTP. (например Chrony)
8. Узел для развертывания Rancher (В отказоустойчивой конфигурации Вам потребуются 3 узла, но для демострации концепции достаточно одного).
...
_Резальтат, у Вас получится установить RKE2 (Kuberntes) и установить в него SUSE Rancher используя локальный Registry и без подключения к интернет_
